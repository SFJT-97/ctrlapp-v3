...
export default function ReportEventIA (args) {
  const { defaultValues, ticketsAcount, name, responseJSON } = args
  const {
  ...
  const [dateTimeEvent, setDateTimeEvent] = useState(getFormatedDate(new Date(), 'YYYY/MM/DD h:m'))
  // const [dateEvent, setDateEvent] = useState(new Date().toLocaleDateString())
  // const [timeEvent, setTimeEvent] = useState(new Date().toLocaleTimeString())
  ...
  const handleSubmit = async () => {
    ...
    if (eventClassification === undefined || eventSubType === undefined || riskQualification === undefined || eventType === undefined ||
      ...
      // console.log(dateTimeEvent instanceof Date)
    ...
    let mMedia = []
    if (image1 !== undefined && image1 !== '') mMedia.push(uploadFile(image1))
    if (image2 !== undefined && image2 !== '') mMedia.push(uploadFile(image2))
    if (image3 !== undefined && image3 !== '') mMedia.push(uploadFile(image3))
    if (video !== undefined && video !== '') mMedia.push(uploadFile(video, false))
  ...
  useEffect(() => {
    ...
    if (responseJSON) {
      ...
      setDateTimeEvent(responseJSON.dateTimeEvent)
      setImage1(tempImg1)
      setImage2(tempImg2)
      setImage3(tempImg3)
      SetVideo(tempVideo)
    ...
    return (
      <ScrollView nestedScrollEnabled>
        <View style={{ rowGap: 30, marginHorizontal: 15, marginTop: 25, marginBottom: 60 }}>
          ...
          <EventClassification allTicketNewClassification={allTicketNewClassification} setEventClassification={setEventClassification} eventType={eventType} fromAIReport={fromAIReport} eventClassificationValue={eventClassification} />
          ...
          <DateTimeDialog dateTimeEvent={dateTimeEvent} setDateTimeEvent={setDateTimeEvent} isIAEvent={fromAIReport} />
          ...
          <RiskQualification allRiskQualifications={allRiskQualifications} setRiskQualification={setRiskQualification} riskQualificationValue={riskQualification} fromAIReport={fromAIReport} />

          <SolutionState allSolutionsStates={allSolutionsStates} SetSolutionState={SetSolutionState} solutionStateValue={solutionState} fromAIReport={fromAIReport} />

          <Button compact style={{ padding: 5, marginTop: 30, backgroundColor: 'green' }} mode='contained' onPress={handleSubmit}>
            Submit Event
          </Button>
